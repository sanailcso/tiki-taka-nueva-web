...
export function AdminShell({ initial, revisions: initialRevisions, media: initialMedia, userName }: { initial: Initial; revisions: ContentRevision[]; media: MediaAsset[]; userName: string }) {
  ...
  function renderSection() {
    ...
    if (section === "areas") return <><SectionHead eyebrow="Sección 04" title="Áreas de negocio" description="Presentación de las tres líneas del mundo presencial." /><div className="cms-card cms-grid-2"><Field label="Antetítulo" value={content.areas.kicker} onChange={(v) => mutate((d) => { d.areas.kicker = v; })} /><Field label="Titular" value={content.areas.title} onChange={(v) => mutate((d) => { d.areas.title = v; })} /><AreaField label="Descripción" value={content.areas.description} onChange={(v) => mutate((d) => { d.areas.description = v; })} /></div><div className="cms-stack">{content.areas.cards.map((card, index) => <article className="cms-card" key={index}><h2>Área {index + 1}</h2><div className="cms-grid-2"><Field label="Antetítulo" value={card.eyebrow} onChange={(v) => mutate((d) => { d.areas.cards[index].eyebrow = v; })} /><Field label="Título" value={card.title} onChange={(v) => mutate((d) => { d.areas.cards[index].title = v; })} /><AreaField label="Descripción" value={card.description} onChange={(v) => mutate((d) => { d.areas.cards[index].description = v; })} /><Field label="Texto del enlace" value={card.label || ""} onChange={(v) => mutate((d) => { d.areas.cards[index].label = v; })} /><Field label="Destino del enlace" value={card.href || ""} onChange={(v) => mutate((d) => { d.areas.cards[index].href = v; })} /></div></article>)}</div></>;

    if (section === "finder") return <><SectionHead eyebrow="Sección 05" title="Salones y mapa" description="Edita la introducción y cada ubicación que aparece en el localizador." /><div className="cms-card cms-grid-2"><Field label="Antetítulo" value={content.finder.kicker} onChange={(v) => mutate((d) => { d.finder.kicker = v; })} /><Field label="Titular" value={content.finder.title} onChange={(v) => mutate((d) => { d.finder.title = v; })} /><AreaField label="Descripción" value={content.finder.description} onChange={(v) => mutate((d) => { d.finder.description = v; })} /><Field label="Cifra" value={content.finder.count} onChange={(v) => mutate((d) => { d.finder.count = v; })} /><Field label="Texto de la cifra" value={content.finder.countLabel} onChange={(v) => mutate((d) => { d.finder.countLabel = v; })} /><Field label="Directorio oficial" value={content.finder.directoryUrl} onChange={(v) => mutate((d) => { d.finder.directoryUrl = v; })} /></div><div className="cms-list-head"><div><h2>{content.salons.length} salones</h2><p>El nombre, la comunidad, las coordenadas y el enlace de Google Maps son editables.</p></div><Button onClick={() => mutate((d) => { d.salons.unshift({ name: "Nuevo salón", region: "Murcia", href: "", lat: 37.98, lng: -1.13 }); })}><Plus />Añadir salón</Button></div><label className="cms-search"><Search /><Input placeholder="Buscar salón" value={salonQuery} onChange={(e) => setSalonQuery(e.target.value)} /></label><div className="cms-stack">{filteredSalons.map(({ salon, index }) => <article className="cms-card cms-salon" key={`${salon.name}-${index}`}><div className="cms-repeater-head"><strong>Tiki Taka {salon.name}</strong><Button size="icon" variant="ghost" onClick={() => mutate((d) => { d.salons.splice(index, 1); })}><Trash2 /></Button></div><div className="cms-grid-3"><Field label="Nombre" value={salon.name} onChange={(v) => mutate((d) => { d.salons[index].name = v; })} /><label className="cms-field"><span>Comunidad</span><select value={salon.region} onChange={(e) => mutate((d) => { d.salons[index].region = e.target.value as typeof salon.region; })}><option>Murcia</option><option>Comunidad Valenciana</option><option>Castilla-La Mancha</option><option>Andalucía</option></select></label><Field label="Enlace de Maps" value={salon.href} onChange={(v) => mutate((d) => { d.salons[index].href = v; })} /><Field label="Latitud" type="number" value={salon.lat} onChange={(v) => mutate((d) => { d.salons[index].lat = Number(v); })} /><Field label="Longitud" type="number" value={salon.lng} onChange={(v) => mutate((d) => { d.salons[index].lng = Number(v); })} /></div></article>)}</div></>;
