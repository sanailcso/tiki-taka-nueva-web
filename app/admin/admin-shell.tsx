...
type Section = "overview" | "identity" | "hero" | "intro" | "play" | "areas" | "finder" | "history" | "commitment" | "jobs" | "footer" | "legal" | "analytics" | "seo" | "motion" | "media" | "security" | "revisions";
...
const nav: Array<{ id: Section; label: string; icon: typeof LayoutDashboard }> = [
  ...
  { id: "footer", label: "Contacto y enlaces", icon: Link2 }, { id: "legal", label: "Textos legales", icon: ShieldCheck },
  { id: "analytics", label: "Analytics", icon: CircleGauge }, { id: "seo", label: "SEO", icon: Globe2 },
...
export function AdminShell({ initial, revisions: initialRevisions, media: initialMedia, userName }: { initial: Initial; revisions: ContentRevision[]; media: MediaAsset[]; userName: string }) {
  ...
  function renderSection() {
    ...
    if (section === "commitment") return <><SectionHead eyebrow="Sección 07" title="Juego responsable" description="Mensaje legal y reputacional de la marca." /><div className="cms-card cms-grid-2"><Field label="Antetítulo" value={content.commitment.kicker} onChange={(v) => mutate((d) => { d.commitment.kicker = v; })} /><Field label="Titular" value={content.commitment.title} onChange={(v) => mutate((d) => { d.commitment.title = v; })} /><AreaField label="Descripción" value={content.commitment.description} onChange={(v) => mutate((d) => { d.commitment.description = v; })} /></div></>;
    ...
    if (section === "footer") return <><SectionHead eyebrow="Pie de página" title="Contacto y enlaces" description="Datos corporativos y canal ético. Las páginas legales se editan desde «Textos legales»." /><div className="cms-card cms-grid-2"><Field label="Lema" value={content.footer.tagline} onChange={(v) => mutate((d) => { d.footer.tagline = v; })} /><Field label="Correo" value={content.footer.email} onChange={(v) => mutate((d) => { d.footer.email = v; })} /><Field label="Teléfono" value={content.footer.phone} onChange={(v) => mutate((d) => { d.footer.phone = v; })} /><AreaField label="Dirección" value={content.footer.address} onChange={(v) => mutate((d) => { d.footer.address = v; })} /><Field label="Canal ético" value={content.footer.ethicsUrl} onChange={(v) => mutate((d) => { d.footer.ethicsUrl = v; })} /></div></>;

    if (section === "legal") return <><SectionHead eyebrow="Cumplimiento" title="Textos legales" description="Datos comunes del aviso legal, la política de privacidad y la política de cookies." /><div className="cms-card"><h2>Tiki Taka Games</h2><div className="cms-grid-2"><Field label="Fecha de actualización" value={content.legal.updatedAt} onChange={(v) => mutate((d) => { d.legal.updatedAt = v; })} /><Field label="Razón social" value={content.legal.companyName} onChange={(v) => mutate((d) => { d.legal.companyName = v; })} /><Field label="NIF" value={content.legal.taxId} onChange={(v) => mutate((d) => { d.legal.taxId = v; })} /><Field label="Teléfono legal" value={content.legal.phone} onChange={(v) => mutate((d) => { d.legal.phone = v; })} /><Field label="Correo general" value={content.legal.email} onChange={(v) => mutate((d) => { d.legal.email = v; })} /><Field label="Correo de privacidad" value={content.legal.privacyEmail} onChange={(v) => mutate((d) => { d.legal.privacyEmail = v; })} /><AreaField label="Domicilio" value={content.legal.address} onChange={(v) => mutate((d) => { d.legal.address = v; })} /><AreaField label="Datos registrales" value={content.legal.registry} onChange={(v) => mutate((d) => { d.legal.registry = v; })} /></div></div><div className="cms-card"><h2>Protección de datos</h2><div className="cms-grid-2"><Field label="Empresa asesora / DPO" value={content.legal.dpoName} onChange={(v) => mutate((d) => { d.legal.dpoName = v; })} /><Field label="NIF de la empresa asesora" value={content.legal.dpoTaxId} onChange={(v) => mutate((d) => { d.legal.dpoTaxId = v; })} /><Field label="Correo para ejercer derechos" value={content.legal.dpoEmail} onChange={(v) => mutate((d) => { d.legal.dpoEmail = v; })} /><AreaField label="Finalidad del tratamiento" value={content.legal.contactPurpose} onChange={(v) => mutate((d) => { d.legal.contactPurpose = v; })} /><AreaField label="Procedencia de los datos" value={content.legal.dataSource} onChange={(v) => mutate((d) => { d.legal.dataSource = v; })} /><AreaField label="Categorías de datos" value={content.legal.dataCategories} onChange={(v) => mutate((d) => { d.legal.dataCategories = v; })} /><AreaField label="Ejercicio de derechos" value={content.legal.rightsText} onChange={(v) => mutate((d) => { d.legal.rightsText = v; })} /></div></div></>;

    if (section === "analytics") return <><SectionHead eyebrow="Medición" title="Google Analytics" description="Accesos de control y estado de la medición de la web pública." /><div className="cms-metrics"><article><CircleGauge /><span>Etiqueta instalada</span><strong>G-X6XCFS15PC</strong><small>Google Analytics 4 · Web corporativa</small></article><article><ShieldCheck /><span>Consentimiento</span><strong>Activo</strong><small>La medición comienza cuando el visitante acepta las cookies analíticas.</small></article><article><Globe2 /><span>Dominio medido</span><strong>www.tikitaka.es</strong><small>Flujo web corporativo</small></article></div><div className="cms-quick"><div><h2>Panel de medición</h2><p>Abre Google Analytics para consultar usuarios, páginas, adquisición y actividad en tiempo real con la cuenta autorizada.</p></div><Button onClick={() => window.open("https://analytics.google.com/analytics/web/", "_blank", "noopener,noreferrer")}>Abrir Google Analytics <ExternalLink /></Button><Button variant="outline" onClick={() => window.open("https://tagassistant.google.com/", "_blank", "noopener,noreferrer")}>Comprobar etiqueta <ExternalLink /></Button><Button variant="outline" onClick={() => window.open("https://www.tikitaka.es/", "_blank", "noopener,noreferrer")}>Abrir web pública <ExternalLink /></Button></div></>;
